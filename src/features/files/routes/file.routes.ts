...
export function createFileRoutes(fileService: FileService, streamService: StreamService): Router {
  ...
  router.get('/gallery', async (req, res, next) => {
    try {
      ...
      const sortBy = req.query.sortBy as string || 'filename'; // Default to filename for natural ordering
      const sortOrder = req.query.sortOrder as string || 'ASC'; // Default to ASC for natural ordering
  ...
  router.get('/:id/play', async (req, res, next) => {
    ...
    const { CacheService } = await import('../../../shared/services/cache.service.js');
    ...
    try {
      ...
      // Cache key for this specific file/range
      const cacheKey = range 
        ? `stream:${id}:${start}-${end}` 
        : `stream:${id}:full`;

      // Try to serve from cache first (only for full file or common ranges)
      // Don't cache arbitrary ranges as they can be infinite combinations
      const shouldCache = !range || (start === 0 && end === file.size - 1);
      
      if (shouldCache) {
        const cachedStream = await cacheService.getStream(cacheKey);
        if (cachedStream) {
          logger.info({ fileId: id, cacheKey }, 'Serving from cache');
          ...
          try {
            ...
            logger.error({ error, fileId: id }, 'Error serving from cache');
      ...
      let { stream } = await fileService.downloadFile(id, start, end, signal);
      
      // Wrap stream with caching if appropriate
      // Cache full files or first chunk for both images and videos
      // Images: typically < 10MB, cache all
      // Videos: cache if < 100MB
      const isImage = file.mimeType?.startsWith('image/');
      const isVideo = file.mimeType?.startsWith('video/');
      const imageSizeLimit = 20 * 1024 * 1024; // 20MB for images
      const videoSizeLimit = 100 * 1024 * 1024; // 100MB for videos
      
      const shouldCacheBySize = isImage 
        ? file.size < imageSizeLimit 
        : (isVideo && file.size < videoSizeLimit);
      
      if (shouldCache && shouldCacheBySize) {
        const cacheTTL = isImage ? 7200 : 3600; // Images: 2 hours, Videos: 1 hour
        stream = cacheService.cacheStream(cacheKey, stream, cacheTTL);
        logger.info({ fileId: id, cacheKey, size: file.size, type: isImage ? 'image' : 'video' }, 'Caching stream for future requests');
      }
